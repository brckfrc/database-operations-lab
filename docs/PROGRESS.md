...
<!-- Section 5b - LB-HA (HAProxy redundancy): VERIFIED. Second HAProxy added on AWS (same haproxy.cfg, host net). Both LBs route correctly (AWS:5000→primary, AWS:5001→replica). LB switch proven via libpq multi-host conn string (host=10.10.0.1,10.10.0.2 :5000, target_session_attrs=read-write): stopped Contabo HAProxy → same connection auto-routed through AWS HAProxy to primary, write uninterrupted. Both LBs back up. Removes LB-layer SPOF. Repo: scripts/05_lb_failover_test.sh. README §Küme/Mimari/§5/§8 updated (both nodes now run HAProxy). -->
